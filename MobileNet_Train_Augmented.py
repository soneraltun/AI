# -*- coding: utf-8 -*-
"""
Created on Tue Dec 24 17:15:05 2024
...
checkpoint = ModelCheckpoint("MobileNet_model_Augmented.h5", monitor='val_loss', 
...
def focal_loss(alpha=0.25, gamma=2.0):
    ...
    Args:
        alpha (float): Sınıf ağırlığı
        gamma (float): Farklı örneklere odaklanma derecesi
    Returns:
        loss (function): Focal loss fonksiyonu
    """
    def loss(y_true, y_pred):
        # Tahminleri kırp (numerik stabilite için)
...
" compile... "
model.compile(Adam(learning_rate=l_rate), loss=focal_loss(alpha=0.25, gamma=2.0),
...
plot_loss(history, "Augmented MobileNet_model Loss Function")
...
def save_model(model, name):
        ...
        with open(fname, "w") as json_file: 
            ...
            print("Saved MobileNet_model_Augmented to disk")
            
save_model(model,'MobileNet_model_Augmented')
...
plt.legend()
plt.grid(True)
accuracyPATH=os.path.join(ResultPATH, "Augmented MobileNet Accuracy.png")
...
plt.title('Augmented Model Loss Function')
plt.ylabel('Loss')
plt.xlabel('Epoch')
plt.legend()
plt.grid(True)
lossPATH=os.path.join(ResultPATH, "Augmented MobileNet Loss Function.png")
